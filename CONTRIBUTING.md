...
跨平台共享核心 P1.2b 固定使用 Rust 1.97.1 和标准库，接入双端生产构建、一次性启动探针与字节数/速率格式化影子比较，但最终输出仍由原生算法决定，详细边界见[跨平台共享核心技术方案](docs/跨平台共享核心技术方案.md)。rustup 必须能提供仓库锁定的主机工具链和目标架构；不得提交 `.build/`、`SharedCore/target/`、静态库或 DLL。
