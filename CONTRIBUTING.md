...
跨平台共享核心固定使用 Rust 1.97.1 和标准库。P1.3 双端运行态门禁已于 2026-08-12 通过；P1.4-2 已把字节数切换为共享优先、原生懒回退，两类速率继续使用受保护主路径。后续只按[懒原生回退方案](docs/跨平台共享核心P1.4懒原生回退技术方案.md)逐格式推进。详细基线见[共享核心技术方案](docs/跨平台共享核心技术方案.md)与[P1.3 验收记录](docs/跨平台共享核心P1.3验收记录-2026-08-12.md)。rustup 必须能提供仓库锁定的主机工具链和目标架构；不得提交 `.build/`、`SharedCore/target/`、静态库或 DLL。
