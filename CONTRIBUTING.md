...
跨平台共享核心固定使用 Rust 1.97.1 和标准库。P1.3 与 P1.4 双端运行态门禁均已于 2026-08-12 通过；字节数、完整速率和紧凑速率保持共享优先、原生懒回退。详细基线见[共享核心技术方案](docs/跨平台共享核心技术方案.md)、[懒原生回退方案](docs/跨平台共享核心P1.4懒原生回退技术方案.md)与[P1.4 验收记录](docs/跨平台共享核心P1.4验收记录-2026-08-12.md)。rustup 必须能提供仓库锁定的主机工具链和目标架构；不得提交 `.build/`、`SharedCore/target/`、静态库或 DLL。

P2-3 生产影子与 P2-5 受保护主路径双端门禁均已于 2026-08-12 通过，见[代理分类技术方案](docs/跨平台共享核心P2代理分类技术方案.md)与[P2-5 验收记录](docs/跨平台共享核心P2-5验收记录-2026-08-12.md)。P2-6 已让共享识别成功时不再计算原生分类；未识别类型和所有共享错误必须且只能执行一次原生回退，P2-7 实机步骤见[运行态验收指南](docs/跨平台共享核心P2-7运行态验收指南.md)。不得删除原生分类、P2-4 路由或差分测试，也不得把 DIRECT、REJECT 或未知静默并入 Proxy。
