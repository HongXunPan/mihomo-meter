...
P2-3 双端生产影子运行态门禁已于 2026-08-12 通过，见[代理分类技术方案](docs/跨平台共享核心P2代理分类技术方案.md)与[验收记录](docs/跨平台共享核心P2-3验收记录-2026-08-12.md)。进入 P2-4 后仍须逐次计算原生分类，只有共享结果与原生结果精确一致才允许返回共享分类；未识别类型和共享错误必须回退原生实现，不得把 DIRECT、REJECT 或未知静默并入 Proxy。
