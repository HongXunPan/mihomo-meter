...
Mihomo Meter 是一款原生 macOS 状态栏应用，通过 Mihomo Controller API 统计真实经过代理出口的实时网速、本机累计流量，以及机场订阅余额和累计用量走势。
...
- 状态栏弹层以原生扁平分区展示剩余流量和总消耗折线面积图，Y 轴按当前范围的总消耗极值动态缩放；每个连续周期段以范围内首个快照为基线，用青色和紫色面积分别堆叠该点之后的下载、上传增量，顶端保持为真实总消耗。图表上方汇总本范围内可比较区间的下载、上传与合计新增，悬停明细展示选中快照的累计绝对值。完整统计窗口通过侧边栏切换 Proxy 流量与订阅余额，支持 24 小时、7 天、30 天、12 月范围、多 Profile 同屏、真实快照悬停明细和放大查看
