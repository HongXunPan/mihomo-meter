...
- 状态栏快速菜单以整行着色的只读进度轨道展示多个 Profile，并给出固定 7 天口径的预计可用天数；查询失败、代理不可用或账本异常优先覆盖预测。趋势子菜单默认展示当前 Profile；Profile、24 小时/7 天范围和遵守冷却与代理约束的立即查询使用保持菜单展开的普通按钮，图表保持只读。完整统计窗口以自适应卡片同屏展示并支持单项或全部查询
