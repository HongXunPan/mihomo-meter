...
Mihomo Meter 是一款原生 macOS 状态栏应用，通过 Mihomo Controller API 统计真实经过代理出口的实时网速、本机累计流量，以及机场订阅余额和区间新增用量走势。
...
- 使用独立的 `quota.sqlite3` 保存机场累计配额快照、周期和套餐变化事件；上传、下载增量及小时、天、周、月柱在读取时动态派生
- 预计耗尽只在样本跨度、周期确认和数据新鲜度满足条件时输出，并明确说明无法预测的原因
- 状态栏弹层以原生扁平分区展示剩余流量和上传、下载新增用量堆叠图，默认查看最近 24 小时并优先按自然小时聚合；查询分钟不决定柱位置，数据不足时提升到更粗自然桶。完整统计窗口通过侧边栏切换 Proxy 流量与订阅余额，主卡片按 24 小时小时柱、7 天日柱、30 天日柱、12 月月柱等距展示，支持悬停明细，并可放大后切换粒度
...
- 状态栏弹层以无圆角卡片背景的单开手风琴展示多个 Profile，默认只展开当前 Profile；完整统计窗口以自适应卡片同屏展示并支持单项或全部查询
