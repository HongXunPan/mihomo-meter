...
用户通过 Windows 文件夹选择器明确选择根部含 `profiles.yaml` 的目录。选择弹窗可以列出已经核对兼容的客户端常见路径；只有用户点击对应客户端的“打开目录”时才把该路径作为选择器建议目录，底部“浏览其他目录”不传入建议路径。该提示不扫描或探测客户端目录，也不扩展解析格式。解析器只读取用户最终所选目录的根文件，限制 2 MiB，拒绝重解析点，忽略未知字段并按 UID 建立身份。Profile 改名保持身份；URL 变化只更新应用本地 HMAC-SHA256 指纹并重新进入查询计划。原始 URL 只存在于本次内存目录快照。
