...
用户通过 Windows 文件夹选择器明确选择根部含 `profiles.yaml` 的目录。选择弹窗可以列出已经核对兼容的客户端常见路径；只有用户点击对应客户端的“打开目录”时才把该路径作为选择器建议目录，底部“浏览其他目录”不传入建议路径。该提示不扫描或探测客户端目录，也不扩展解析格式。解析器只读取用户最终所选目录的根文件，限制 2 MiB，拒绝重解析点，忽略未知字段并按 UID 建立身份。Profile 改名保持身份；URL 变化只更新应用本地 HMAC-SHA256 指纹并重新进入查询计划。Profile 缺失或 URL 非 HTTPS 时标记不支持，恢复受支持 Profile 后恢复追踪；清除目录访问只停止查询，不改写用户追踪意图。原始 URL 只存在于本次内存目录快照。
...
主窗口缓存“订阅余额”工作台，展示轻量模式确认、Profile 追踪与间隔、单项/全部查询、周期确认、四档趋势和清空。卡片补充系统本地化到期日、最近三条事实变化、待确认周期说明和失败后的自动重试时间；摘要时间使用相对文本。放大趋势可独立切换四档范围。图表使用 WinUI 原生 Canvas/Shape，以范围首尾真实时间为横轴，提供 X/Y 轴、下载/上传增量堆叠、周期断点，以及包含真实起止时间、增量和间隔的指针与键盘明细。
