...
`scripts/build_windows_installer.ps1` 只负责调用已锁定的 NSIS 编译器、传入版本/载荷/输出路径并复核安装器版本，不复制发布、测试或 ZIP 职责。NSIS 脚本固定放在 `platform/windows/installer/MihomoMeter.nsi`，不得引入额外插件。
